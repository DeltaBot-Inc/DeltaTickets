...
module.exports = async (bot, reaction, user) => {
  ...
  .setTitle(`🎫 DeltaTickets | Nouveau ticket`)
  .setFooter(`DeltaTickets | Crée par DeltaBot Inc.`, bot.user.displayAvatarURL())
  .setDescription("<a:fleche:750809388414926932> Veuillez expliquer la raison de votre demande. Un membre de l'équipe prendra en charge votre ticket sous peu.\n<a:fleche:750809388414926932> Faire `t*close` pour fermer votre ticket");
  ...
  if(message.embeds.length === 1 && message.embeds[0].title === 'Système de Ticket' && message.embeds[0].description === 'Réagissez avec 🎟️ pour créer un ticket.'){
    if(reaction.emoji.name === "🎟️"){
      if(!message.guild.channels.cache.find(c => c.name === `ticket-${usr[0]}${usr[1]}${usr[2]}${usr[3]}`)){
        ...
        .setAuthor(`🎫 DeltaTickets | Ticket Ouvert`)
        .setTimestamp()
        .setColor(color.none)
        .setFooter(`DeltaTickets | Crée par DeltaBot Inc.`, bot.user.displayAvatarURL())
        .setDescription(`Un utilisateur à ouvert un ticket et attend qu'on s'occupe de sa demande.`)
        .addField(`Informations`, `<a:fleche:750809388414926932> **Utilisateur :** \`${user.tag}\`\n<a:fleche:750809388414926932> **ID :** \`${user.id}\`\n<a:fleche:750809388414926932> **Ticket :** ${channel}\n<a:fleche:750809388414926932> **Date :** \`${dateFormat(new Date(), "dd/mm/yyyy - HH:MM:ss")}\``);
  ...
  if(message.embeds.length === 1 && message.embeds[0].title === '🎟️ | Ticket Terminé' && message.embeds[0].description === `Réagissez avec \\🗑️ pour fermer le ticket ou ne réagissez pas si vous avez d'autres demandes.`){
    if(reaction.emoji.name === "🗑️"){
      if(user.id === db.get(`ticket.${message.channel.name}.user`)){
        ...
        .setAuthor(`🗑️ DeltaTickets | Ticket Fermé`)
        .setColor(color.none)
